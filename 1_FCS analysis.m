...
for seed=1:6
    pred_table                                      = readtable(strcat('F:\OneDrive - 北京师范大学\project2\output_distance\FCS_k=3\seedFC\seedFC',num2str(seed),'\seedFC_predvalue.txt'));
    pred_table                                      = sortrows(pred_table,'Var1','ascend');
    pred_FCs                                        = table2array(pred_table(:,2:end));
    
    scan_age_pred                                   = load('F:\OneDrive - 北京师范大学\project2\output_distance\FCs\scan_age.txt');
